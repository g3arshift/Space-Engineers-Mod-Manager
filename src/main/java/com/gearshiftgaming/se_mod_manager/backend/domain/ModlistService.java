...
public class ModlistService {
	...
	private Result<String[]> scrapeModInformation(String modId, ModType modType) throws IOException {
		...
		if (modType == ModType.STEAM) {
			modScrapeResult = scrapeSteamMod(modId);
		} else {
			modScrapeResult = scrapeModIoMod(modId);
		}
			...
			if (pageSource != null) {
				...
				if (checkIfModIsMod(ModType.MOD_IO, modPage)) {
					...
					modInfo[1] = switch (lastUpdatedQuantifier) {
						...
						case "d": yield LocalDate.now().minusDays(duration).format(DateTimeFormatter.ofPattern("MMM d',' yyyy"));
						case "y": yield LocalDateTime.now().minusYears(duration).format(DateTimeFormatter.ofPattern("yyyy"));
